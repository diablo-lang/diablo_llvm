...
let test_function () =
  ...
  in
  let typed_ast = Hm.convert_to_typed_ast parsed_ast in
  Alcotest.(check string) "Function type check" "Program return type: void" (print_typed_ast typed_ast)
...
let test_if () =
  ...
  in
  let typed_ast = Hm.convert_to_typed_ast parsed_ast in
  Alcotest.(check string) "If type check" "Program return type: void" (print_typed_ast typed_ast)
