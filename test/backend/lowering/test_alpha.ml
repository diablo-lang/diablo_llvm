...
and pp_expr ppf = function
  ...
  | Lambda (params, body, _) ->
      Fmt.pf ppf "\\%a -> %a"
        (Fmt.list (fun ppf (name, _) -> Fmt.pf ppf "%s" name))
        params pp_expr body
  ...
  | UnOp (op, expr, _) -> Fmt.pf ppf "%s%a" (string_of_un_op op) pp_expr expr
  | List (exprs, _) -> Fmt.pf ppf "[%a]" (Fmt.list pp_expr) exprs
  | If (cond_expr, then_expr, else_expr, _) ->
      Fmt.pf ppf "if %a then %a else %a" pp_expr cond_expr pp_expr then_expr
        pp_expr else_expr
...
let test_simple_let () =
  ...
  check
    (testable pp_program ( = ))
    "Simple let renaming" expected (Alpha.rename program)
...
let test_lambda_shadowing () =
  ...
  check
    (testable pp_program ( = ))
    "Lambda shadowing" expected (Alpha.rename program)
...
let test_lambda_reference_outside () =
  ...
  check
    (testable pp_program ( = ))
    "Lambda referencing outer variable" expected (Alpha.rename program)
