...
module Infer = struct
  ...
  let rec infer_block env block =
    ...
    | Parsed_ast.Block exprs ->
      List.fold_left
        (fun (s, _) expr -> let s', t' = infer (List.map (fun (x, t) -> (x, Subst.apply s t)) env) expr in (Subst.compose s s', t'))
        (Subst.empty, Type.TVoid)
        exprs
  ...
  and infer env expr =
    ...
    | Parsed_ast.If (cond, then_branch, else_branch) ->
      ...
      let s3, t2 = infer_block (List.map (fun (x, t) -> (x, Subst.apply s2 t)) env) then_branch in
      let s4, t3 = infer_block (List.map (fun (x, t) -> (x, Subst.apply s2 t)) env) else_branch in
...
let type_check (Parsed_ast.Program (_import_stmts, functions, main_expr)) =
  ...
  let convert_to_typed_ast (Parsed_ast.Program (_import_stmts, functions, main_block)) =
