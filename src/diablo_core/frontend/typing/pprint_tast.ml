...
let rec string_of_expr = function
  ...
  | Unit -> "()"
  | Call (f, args, ty) -> 
      "Call(" ^ string_of_expr f ^ ", [" ^ String.concat ", " (List.map string_of_expr args) ^ "], " ^ string_of_ty ty ^ ")"
  ...
  | If (cond, then_expr, else_expr, ty) ->
      "If(" ^ string_of_expr cond ^ ", " ^ string_of_expr then_expr ^ ", " ^ string_of_expr else_expr ^ ", " ^ string_of_ty ty ^ ")"
  | BinOp (op, left, right, ty) ->
      "BinOp(" ^ string_of_bin_op op ^ ", " ^ string_of_expr left ^ ", " ^ string_of_expr right ^ ", " ^ string_of_ty ty ^ ")"
  | UnOp (op, expr, ty) ->
      "UnOp(" ^ string_of_un_op op ^ ", " ^ string_of_expr expr ^ ", " ^ string_of_ty ty ^ ")"
  | List (elements, ty) ->
      "List([" ^ String.concat ", " (List.map string_of_expr elements) ^ "], " ^ string_of_ty ty ^ ")"
