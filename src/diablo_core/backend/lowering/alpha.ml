...
module Alpha = struct
  ...
  let rename =
    let rec rename_expr env : Desugared_ast.expr -> Desugared_ast.expr =
      function
      ...
      | Desugared_ast.Lambda (params, body, ty) ->
          let params' =
            List.map (fun (name, ty) -> (NameGen.fresh name, ty)) params
          in
          let env' =
            List.fold_left2
              (fun env (orig_name, _) (fresh_name, _) ->
      ...
      | Desugared_ast.If (cond_expr, then_expr, else_expr, ty) ->
          If
            ( rename_expr env cond_expr,
              rename_expr env then_expr,
              rename_expr env else_expr,
              ty )
      | Desugared_ast.BinOp (op, lhs_expr, rhs_expr, ty) ->
          BinOp (op, rename_expr env lhs_expr, rename_expr env rhs_expr, ty)
      | Desugared_ast.UnOp (op, expr, ty) -> UnOp (op, rename_expr env expr, ty)
    ...
    let rename_program : Desugared_ast.program -> Desugared_ast.program =
      function
          ...
          let _, decls' =
            List.fold_left
              (fun (env, acc) decl ->
                ...
                (env', acc @ [ decl' ]))
              (SM.empty, []) top_level_declarations
          in
          Program (decls', ty)
    in
